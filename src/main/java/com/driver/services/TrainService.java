...
public class TrainService {
    ...
    public Integer calculateAvailableSeats(SeatAvailabilityEntryDto seatAvailabilityEntryDto){
        ...
        //The seat is available only between A to C and A to B. If a seat is empty
        // between 2 station it will be counted to our final ans
        //even if that seat is booked post the destStation or before
        // the boardingStation
        //Inshort : a train has totalNo of seats and there are tickets
        ...
        for(Ticket t : ticketList){
            ...
            if(map.get(seatAvailabilityEntryDto.getToStation().toString()) <= map.get(seatAvailabilityEntryDto.getToStation().toString())){
                count++;
            } else if (map.get(seatAvailabilityEntryDto.getFromStation()) >= map.get(seatAvailabilityEntryDto.getToStation().toString())){
                count++;
            }
        }
        return  count+2;
    ...
    public Integer calculatePeopleBoardingAtAStation(Integer trainId,Station station) throws Exception{
        ...
        //We need to find out the number of people who will be boarding
        // a train from a particular station
    ...
    public List<Integer> trainsBetweenAGivenTime(Station station, LocalTime startTime, LocalTime endTime){
        ...
        List<Train> trains1 = trainRepository.findAll();
        for (Train train: trains1) {
            ...
            for (int i = 0; i <ans.length ; i++) {
                if(Objects.equals(ans[i],String.valueOf(station))){
                    ...
                    int lastTimeMax = (endTime.getHour()*60 + endTime.getMinute());

                    int departureTimeMin = (train.getDepartureTime().getHour()*60) + train.getDepartureTime().getMinute();
                    int reachingTimeMin = departureTimeMin + (i * 60);
                    if(reachingTimeMin >= startTimeMin && reachingTimeMin <= lastTimeMax)
